                ...
                <div class="knowledge-preview-panel">
                    ...
                    <div class="user-qa-section">
                        <el-collapse v-model="qaCollapseActive">
                            <el-collapse-item title="原始问题" name="question">
    ...
    data() {
        return {
            ...
            knowledgeTypeOptions: [
                { value: 'innerkp_plus', label: '内部知识库' },
                { value: 'cellphone_cnt', label: '手机知识库' },
                { value: 'outerkp_plus', label: '外部知识库' },
                { value: 'pc_cnt', label: 'PC知识库' }
            ],
            // 当前选中的知识库类型
            selectedKnowledgeType: 'innerkp_plus',
        }
    },
    watch: {
        // 监听知识库搜索文本变化
        ...
        async fetchKnowledgeCategories() {
            ...
            try {
                const params = new URLSearchParams({
                    answer_type: this.selectedKnowledgeType
                });
                
                const response = await axios.get(`${baseUrl}/api/feedback/getKnowleCate/?${params.toString()}`, {
                    headers: {
                        'Authorization': API_AUTH_TOKEN
                    }
                });
                ...
                if (response.data && response.data.knowledge_list && response.data.knowledge_list.length > 0) {
                    // 使用新的数据格式，直接从knowledge_list中获取分类
                    this.knowledgeTree = this.transformCategoryTree(response.data.knowledge_list);
